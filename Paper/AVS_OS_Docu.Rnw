...
<<echo=FALSE>>=
load("~/Documents/R workspace/AVS Outside Evidence work/AVSPaperWS.RData")
.First()
@
\begin{figure}[!hbtp]
<<fig = TRUE, echo=FALSE>>=
plot(ContRaD_01b$Curr,ContRaD_01b$Gprofit, main = "Gross Profit by Country/Currency")
text(1,min(DKK_Gprofit),DKK_ContRaD_01b$Comp[DKK_ContRaD_01b$Gprofit==min(DKK_Gprofit)],pos=3,cex=0.5)
text(1,max(DKK_Gprofit),DKK_ContRaD_01b$Comp[DKK_ContRaD_01b$Gprofit==max(DKK_Gprofit)],pos=1,cex=0.5)
text(1,median(DKK_Gprofit),paste(format(median(DKK_Gprofit), digits = 4), " kEUR"),pos=3,cex=0.5)
text(2,max(GBP_Gprofit),GBP_ContRaD_01b$Comp[GBP_ContRaD_01b$Gprofit==max(GBP_Gprofit)],pos=3,cex=0.5)
text(2,min(GBP_Gprofit),GBP_ContRaD_01b$Comp[GBP_ContRaD_01b$Gprofit==min(GBP_Gprofit)],pos=3,cex=0.5)
text(2,median(GBP_Gprofit),paste(format(median(GBP_Gprofit), digits = 4), " kEUR"),pos=3,cex=0.5)
@
\caption {Test}
\end{figure}
\section{Appendices}
